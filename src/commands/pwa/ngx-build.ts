...
$ sfdx pwa:ngx-build
  ...
  private async buildSfdcSources(projectPath: string, pluginSettings: Settings) {
    const ngDirPath = path.resolve(projectPath, pluginSettings.pwaPath);
    const ngProjectPath = path.resolve(ngDirPath, 'dist', pluginSettings.pwaProject);
  ...
  private hasOverrides(settings: Settings, processFlags: BuildFlags): boolean {
    return (
      ...
      (processFlags.ngpath && processFlags.ngpath !== settings.pwaPath) ||
  ...
  private overrideConfig(settings: Settings, processFlags: BuildFlags): Settings {
    ...
    if (processFlags.ngpath) {
      settings.pwaPath = processFlags.ngpath;
    }
