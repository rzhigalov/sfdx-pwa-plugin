...
export default class Init extends SfdxCommand {
 ...
 Starts plugin configuration wizard
  ...
  public async run(): Promise<AnyJson> {
    ...
    const pluginSettings: Settings = await mergeConfigDefaults(projectConfig.getContents());
    ...
    pluginSettings.framework = await this.ux.prompt(
      messages.getMessage('frameworkFlagDescription')
    );

    pluginSettings.pwaPath = await retry(() =>
      this.setAndValidateDir(
        messages.getMessage('pwaPathFlagDescription'),
        pluginSettings.pwaPath,
        projectPath
      )
    );

    if (pluginSettings.framework === 'ng') {
      const ngRootPath = path.join(projectPath, pluginSettings.pwaPath);
      const suggestNgProject =
        pluginSettings.pwaProject ||
        ((await retrieveNgProjectJson(ngRootPath))['defaultProject'] as string);
      pluginSettings.pwaProject = await this.ux.prompt(
